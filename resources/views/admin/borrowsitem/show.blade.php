    ...
    <div class="card shadow-lg border-0">
        ...
        <div class="card-body p-4">
            ...
            <table class="table table-hover table-bordered align-middle">
                ...
                <tbody>
                    @php $index = 1; @endphp
                    @if(is_null($borrowItem->borrow->reader_id))
                    <tr>
                        <th>{{ $index++ }}</th>
                        <th>Tiền thuê</th>
                    ...
                    <tr>
                        <th>{{ $index++ }}</th>
                        <th>Tiền cọc</th>
                        <th class="text-end">{{ number_format($borrowItem->tien_coc) }}</th>
                    </tr>
                    <tr>
                        <th>{{ $index++ }}</th>
                        <th>Tiền ship</th>
...
<tfoot class="table-light fw-bold fs-5">
                    <tr>
                        ...
                        <td class="text-end text-success">{{ number_format($borrowItem->tien_thue + $borrowItem->tien_coc + $borrowItem->tien_ship) }}₫</td>
                    </tr>
                </tfoot>
            <!-- Status Badges -->
                ...
                <div>
                    ...
                    <p><strong>Trạng thái cọc:</strong>
                        ...
                        <span class="badge bg-gradient-{{ $statusColors[$borrowItem->trang_thai_coc] ?? 'secondary' }} text-white px-3 py-2">
                            {{ ucfirst(str_replace('_', ' ', $borrowItem->trang_thai_coc)) }}
                        </span>
                    </p>
                </div>
                <div class="text-end">
...
.badge.bg-gradient-info {
    background: linear-gradient(90deg, #17a2b8, #33c1d3);
}
.badge.bg-gradient-success {
    background: linear-gradient(90deg, #28a745, #71d88e);
}
.badge.bg-gradient-danger {
    background: linear-gradient(90deg, #dc3545, #ff6b6b);
}
.badge.bg-gradient-secondary {
    background: linear-gradient(90deg, #6c757d, #a0a0a0);
}
