...
class InventoryController extends Controller
{
    ...
    public function syncData(Request $request)
    {
        try {
            ...
            // ========== 2. ĐỒNG BỘ ĐẶT TRƯỚC ==========
            // Đảm bảo Reservation đồng bộ với Book và Reader
            $reservations = \App\Models\Reservation::with(['book', 'reader', 'user'])->get();

            foreach ($reservations as $reservation) {
                try {
                    // Kiểm tra Book có tồn tại không
                    if (!$reservation->book) {
                        $results['reservations']['errors'][] = "Reservation #{$reservation->id}: Book không tồn tại";
                        continue;
                    }

                    // Kiểm tra Reader có tồn tại không
                    if (!$reservation->reader) {
                        $results['reservations']['errors'][] = "Reservation #{$reservation->id}: Reader không tồn tại";
                        continue;
                    }

                    // Kiểm tra User có tồn tại không
                    if (!$reservation->user) {
                        $results['reservations']['errors'][] = "Reservation #{$reservation->id}: User không tồn tại";
                        continue;
                    }

                    // Kiểm tra email của Reader và User có khớp không
                    $reader = $reservation->reader;
                    $user = $reservation->user;
                    
                    if ($reader->email !== $user->email) {
                        // Tìm Reader có email khớp với User
                        $correctReader = \App\Models\Reader::where('email', $user->email)->first();
                        if ($correctReader) {
                            $reservation->update(['reader_id' => $correctReader->id]);
                        }
                    }

                    // Cập nhật trạng thái hết hạn nếu cần
                    if ($reservation->isExpired() && $reservation->status === 'pending') {
                        $reservation->update(['status' => 'expired']);
                    }

                    $results['reservations']['synced']++;
                } catch (\Exception $e) {
                    $results['reservations']['errors'][] = "Lỗi đồng bộ Reservation #{$reservation->id}: " . $e->getMessage();
                }
            }
            ...
            $message .= "- Đặt Trước: {$results['reservations']['synced']} bản ghi\n";
            $message .= "- Đánh Giá: {$results['reviews']['synced']} bản ghi";

            if (!empty($results['sold_books']['errors']) || 
                !empty($results['reservations']['errors']) || 
