...
class InventoryReportController extends Controller
{
    ...
    public function borrowReturnReport(Request $request)
    {
        ...
        $totalReturned = Borrow::whereBetween('ngay_tra_thuc_te', [$fromDate, $toDate])
        ...
        while ($currentDate <= $endDate) {
            $dailyStats[] = [
                ...
                'returned' => Borrow::whereDate('ngay_tra_thuc_te', $currentDate->toDateString())
                    ->whereNotNull('ngay_tra_thuc_te')
                    ->count(),
            ];
            $currentDate->addDay();
        ...
        while ($startWeek <= $endWeek) {
            ...
            $weeklyStats[] = [
                ...
                'returned' => Borrow::whereBetween('ngay_tra_thuc_te', [$startWeek->toDateString(), $weekEnd->toDateString()])
                    ->whereNotNull('ngay_tra_thuc_te')
                    ->count(),
            ];
            $startWeek->addWeek();
        ...
        while ($startMonth <= $endMonth) {
            ...
            $monthlyStats[] = [
                ...
                'returned' => Borrow::whereBetween('ngay_tra_thuc_te', [$startMonth->toDateString(), $monthEnd->toDateString()])
                    ->whereNotNull('ngay_tra_thuc_te')
                    ->count(),
            ];
            $startMonth->addMonth();
        ...
        $topBorrowedBooks = Borrow::with('book')
            ->whereBetween('ngay_muon', [$fromDate, $toDate])
    ...
    public function fineReport(Request $request)
    {
        ...
        // Chi tiết khoản phạt
        $fines = Fine::with(['reader', 'borrow.book', 'creator'])
            ->whereBetween('created_at', [$fromDate, $toDate])
            ->orderBy('created_at', 'desc')
            ->paginate(20);
    ...
    public function lateReturnReport(Request $request)
    {
        ...
        $lateReturns = Borrow::with(['reader', 'book', 'reader.department'])
            ->where(function($query) use ($fromDate, $toDate) {
                $query->where('trang_thai', 'Qua han')
                    ->orWhere(function($q) use ($fromDate, $toDate) {
                        $q->where('trang_thai', 'Da tra')
