   ...
   public function showCheckout(Request $request)
{
    ...
    if ($fromUrl) {
        ...
        // Nếu không có khoảng cách và không có địa chỉ, để JS tính tự động khi người dùng nhập
    }
    ...
    return view('borrow-cart.checkout', compact(
        'cart', 'reader', 'items',
        'totalTienCoc', 'totalTienThue', 'totalTienShip', 'tongTien', 'fromUrl', 'walletBalance'
    ));
}
...
public function processCheckout(Request $request)
{
    ...
    try {
        ...
        if ($manualShippingFee !== null) {
            ...
            if ($tinhThanh) $fullAddress .= $tinhThanh . ', Việt Nam';
        ...
        if ($paymentMethod === 'wallet') {
            ...
            if ($wallet->balance < $totalAmount) {
                ...
                return response()->json([
                    'success' => false,
                    'message' => 'Số dư ví không đủ. Số dư hiện tại: ' . number_format($wallet->balance, 0, ',', '.') . '₫. Vui lòng nạp thêm tiền vào ví.'
                ], 400);
            }
